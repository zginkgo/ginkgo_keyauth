PROJECT_NAME := "ginkgo_keyauth"
MAIN_FILE_PAHT := "main.go"
PKG := "github.com/zginkgo/ginkgo_keyauth"
IMAGE_PREFIX := "github.com/zginkgo/ginkgo_keyauth"

MOD_DIR := $(shell go env GOPATH)/pkg/mod
PKG_LIST := $(shell go list ${PKG}/... | grep -v /vendor/ | grep -v redis)
GO_FILES := $(shell find . -name '*.go' | grep -v /vendor/ | grep -v _test.go)

#MCUBE_MODULE := "github.com/infraboard/ginkgo_mcube"
...
pb: ## Copy ginkgo_mcube protobuf files to common/pb
